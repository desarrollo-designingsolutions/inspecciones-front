...
const optionsTable = {
  ...
  actions: {
    changeStatus: {
      url: "/type-document/changeStatus"
    },
    delete: {
      url: "/type-document/delete"
    },
  }
}
...
const goViewView = async (data: any) => {
  router.push({ name: "Type-Document-Form", params: { action: 'view', id: data.id } })
}

const goViewEdit = async (data: any) => {
  router.push({ name: "Type-Document-Form", params: { action: 'edit', id: data.id } })
}

const goViewCreate = async () => {
  router.push({ name: "Type-Document-Form", params: { action: 'create' } })
}
...
const downloadExcel = async () => {
  ...
  const { data, response } = await useAxios("/type-document/excelExport").get({
    params: {
      ...route.query,
      company_id: authenticationStore.company.id
    }
  ...
  if (response.status == 200 && data) {
    downloadExcelBase64(data.excel, "Lista Tipos de documento")
  }
}
</script>
