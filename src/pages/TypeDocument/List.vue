...
const goView = (data: { action: string, id: number | null } = { action: "create", id: null }) => {
  router.push({ name: "Type-Document-Form", params: { action: data.action, id: data.id } })
}
...
const downloadExcel = async () => {
  ...
  const { data, response } = await useApi("/type-document/excelExport").post({
  ...
  if (response.value?.ok && data.value) {
    downloadExcelBase64(data.value.excel, "Lista Tipos de documento")
  }
}
