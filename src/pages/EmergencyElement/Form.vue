...
definePage({
  path: "Emergency-Element-form/:action/:id?",
  name: "Emergency-Element-Form",
  meta: {
...
const submitForm = async (isCreateAndNew: boolean = false) => {
  ...
  if (validation?.valid) {
    ...
    const url = form.value.id ? `/emergency-element/update/${form.value.id}` : `/emergency-element/store`
    ...
    if (response.value?.ok && data.value) {
      ...
      if (data.value.code == 200) {
        ...
        if (isCreateAndNew) {
          ...
          router.push({ name: 'Emergency-Element-List' }); // Redirigir si es "Crear"
        }
      }
    }
  ...
  <div>
    <VCard :disabled="loading.form" :loading="loading.form">
      <VCardTitle class="d-flex justify-space-between">
        <span>
          Información del elemento de emergencia
        </span>
      </VCardTitle>
      <VCardText>
